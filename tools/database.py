...
# ---------------------------------------------------------------------------
# Rules-engine pivot (2026-05-07): helpers for the new `regime_state`,
# `trades` (simplified), and `audit_log` tables. Old helpers above this line
# remain in place until Task 14 of the pivot plan removes them.
#
# Note: the new ``insert_trade`` below uses keyword-only arguments and writes
# the post-pivot ``trades`` schema (symbol/side/qty/fill_price/...). It
# rebinds the module-level ``insert_trade`` name and supersedes the dict-in
# helper above. Old call sites still using the dict form will fail loudly
# at the type/SQL boundary — by design, since the columns they target were
# dropped by ``2026_05_07_rules_engine_pivot.sql``.
# ---------------------------------------------------------------------------
