...
const MARKDOWN_COMPONENTS: Components = {
  ...
  thead: ({ children }) => (
    <thead className="bg-bg-glow/60 text-[11px] uppercase tracking-wide text-muted">{children}</thead>
  ),
  tbody: ({ children }) => <tbody className="divide-y divide-border">{children}</tbody>,
  tr: ({ children }) => <tr className="hover:bg-bg-glow/40">{children}</tr>,
  th: ({ children }) => <th className="px-3 py-2 font-medium">{children}</th>,
  td: ({ children }) => <td className="px-3 py-2 text-fg-2">{children}</td>,
  ul: ({ children }) => <ul className="list-disc space-y-1 pl-5 text-fg-2">{children}</ul>,
  li: ({ children, className }) => <li className={`${className ?? ""} text-fg-2`}>{children}</li>,
  input: ({ checked, disabled }) => (
    <input type="checkbox" checked={checked ?? false} disabled={disabled ?? true} readOnly className="mr-2 accent-pos" />
  ),
  h1: ({ children }) => <h2 className="text-lg font-semibold text-fg">{children}</h2>,
  h2: ({ children }) => <h3 className="text-base font-semibold text-fg">{children}</h3>,
  code: ({ children }) => <code className="rounded bg-border px-1 py-0.5 text-xs text-fg-2">{children}</code>,
};
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 p-6">
      <header className="flex items-baseline justify-between">
        ...
        <Link href="/daily" className="text-xs text-muted hover:text-fg-2">
          ← all days
        </Link>
      </header>
      <article className="space-y-4 text-sm leading-relaxed text-fg-2">
