...
- This PR (PR-A) ships **no cron activation**. `0012_hourly_scans.sql`'s
  ...
  separate PR-B (§9), merged last, behind seven checked gates (§9).
...
| `hourly-check` wired into `deploy-dev.yml`'s JWT-verified deploy step | Done (this PR, #479 T5) | `.github/workflows/deploy-dev.yml` |
...
`0013` and the `hourly-check` function deploy wiring ship together in this PR, so
merging PR-A to `main` triggers `deploy-dev.yml`'s existing order automatically:
...
No manual step is required for this section — it happens on merge. The operator steps
begin at §4.
...
2. #480 (post-fill journaling failure window: bounded retry + reconciliation
   recovery) merged and deployed.
3. Layer-B pin merged from a real capture (§12 appendix) — not the fail-closed
   placeholder this PR ships.
4. T9 evidence (§7) posted on #479, outcome `skipped:market_closed`.
5. T10 evidence (§8) posted on #479, inequality holds with the observed latency.
6. §4's secrets set on `qdaxxsuicyiscdvsdowc`.
7. §5's baseline row present; `bot_config.paused = 'false'` confirmed (§6).
