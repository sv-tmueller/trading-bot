...
- Deploy to **dev** — ORDER MATTERS (final-review finding): `supabase functions deploy daily-check` FIRST, then `supabase db push` (applies 0006). The reverse order opens a window where the new 13:37/14:37 crons invoke the OLD function during market hours — no clock gate, no partial-bar filter — which could trade on today's in-progress bar. (New function + old 22:30 cron is safe: it just gate-exits.) Alternatively do both back-to-back outside 13:30–15:30 UTC on a weekday. Confirm in the dashboard/SQL editor that `cron.job` now lists `daily-check-1337`/`daily-check-1437` and no `daily-check`.
