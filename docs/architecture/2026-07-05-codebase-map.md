...
| **Web dashboard** | Read-only Next.js status page (position, regime, drawdown, kill-switch, trades, audit log, Alpaca holdings); no controls. Deployed on Vercel. | `app/page.tsx` (server-rendered), `lib/supabase.ts`, `lib/alpaca.ts` (GET-only), `lib/auth.ts` + `middleware.ts` (optional Basic auth), `next.config.mjs` |
...
- **Dashboard → migrations' schema.** `web/` reads the same five tables via its own (separate) Supabase and Alpaca clients — no shared code with `_shared`.
